...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"gather_source") == 0) return gather_source;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"gather_source") == 0) return Data::AnyFieldToStringConverter::ToString(&gather_source);
