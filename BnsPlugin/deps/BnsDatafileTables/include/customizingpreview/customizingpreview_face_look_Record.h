...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"face_look_showname") == 0) return face_look_showname;
			if (wcscmp(fieldName, L"face_look_show_duration") == 0) return face_look_show_duration;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"face_look_showname") == 0) return Data::AnyFieldToStringConverter::ToString(&face_look_showname);
			if (wcscmp(fieldName, L"face_look_show_duration") == 0) return Data::AnyFieldToStringConverter::ToString(&face_look_show_duration);
		...
		void AutoCorrectLookupOffsets(const char* lookupDataPtr) {
			...
			if (reinterpret_cast<std::uintptr_t>(face_look_showname) < reinterpret_cast<std::uintptr_t>(lookupDataPtr)) face_look_showname = reinterpret_cast<wchar_t*>(const_cast<char*>(lookupDataPtr) + reinterpret_cast<std::uintptr_t>(face_look_showname));
		}
