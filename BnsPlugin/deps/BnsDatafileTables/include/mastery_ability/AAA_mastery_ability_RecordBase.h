...
#pragma pack(push, 1)
	struct mastery_ability_Record : DrEl
	{
	private:
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"r_ability_rate_max_guard_gauge_equip") == 0) return r_ability_rate_max_guard_gauge_equip;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"r_ability_rate_max_guard_gauge_equip") == 0) return Data::AnyFieldToStringConverter::ToString(&r_ability_rate_max_guard_gauge_equip);
...
#pragma pack(push, 1)
	struct __declspec(align(4)) mastery_ability_RecordPtr // : DrRecordPtr
	{
		...
		//__unaligned __declspec(align(1)) const Data::mastery_ability_Record* _debug;
		bool _makeCopy;
	};
#pragma pack(pop)
}