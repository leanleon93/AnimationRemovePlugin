...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"point_bonus_percent") == 0) return point_bonus_percent;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"point_bonus_percent") == 0) return Data::AnyFieldToStringConverter::ToString(&point_bonus_percent);
