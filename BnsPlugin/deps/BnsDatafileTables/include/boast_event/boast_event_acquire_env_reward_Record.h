...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"boast_from_env") == 0) return boast_from_env;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"boast_from_env") == 0) return Data::AnyFieldToStringConverter::ToString(&boast_from_env);
