...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"limit_hp_min") == 0) return limit_hp_min;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"limit_hp_min") == 0) return Data::AnyFieldToStringConverter::ToString(&limit_hp_min);
