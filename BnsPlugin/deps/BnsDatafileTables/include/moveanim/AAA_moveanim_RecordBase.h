...
#pragma pack(push, 1)
	struct moveanim_Record : DrEl
	{
	private:
		static std::wstring Get_weapon_EnumValue(__int8 value);
		static std::wstring Get_combat_pose_EnumValue(__int8 value);
		static std::wstring Get_stance_EnumValue(__int8 value);
		static std::wstring Get_link_EnumValue(__int8 value);
	public:
		std::any GetField(const wchar_t* fieldName) const {
			if (wcscmp(fieldName, L"moving_blend_time") == 0) return moving_blend_time;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			if (wcscmp(fieldName, L"moving_blend_time") == 0) return Data::AnyFieldToStringConverter::ToString(&moving_blend_time);
