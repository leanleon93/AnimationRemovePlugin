...
#pragma pack(push, 1)
	...
	public:
		std::any GetField(const wchar_t* fieldName) const {
			...
			if (wcscmp(fieldName, L"party_chat_channel") == 0) return party_chat_channel;
		...
		std::wstring GetFieldAsString(const wchar_t* fieldName) {
			...
			if (wcscmp(fieldName, L"party_chat_channel") == 0) return Data::AnyFieldToStringConverter::ToString(&party_chat_channel);
